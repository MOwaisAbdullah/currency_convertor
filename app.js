...
function search(myarray, criteria) {
    return (myarray.filter((item) => item.includes(criteria)));
}
// **************________SEARCH FUNCTION_____************************
...
const myarray = list;
const sorted = search(myarray, search_criteria);
...
const sorted2 = search(myarray, search_criteria_to);
...
async function getrate() {
    ...
    //console.log(rate)
    //console.log(keys)
    let Pkr_Rate = data.rates[`${from}`];
    let Other_Rate = data.rates[`${ANS_to.To}`];
    //console.log(Other_Rate)
    if (ANS_to.To === 'USD') {
        let a = (ANS_to.amount / Pkr_Rate);
        console.log(chalk.cyan.bold(`You Will get ${a} ${ANS_to.To}`));
    }
    else if (ANS_to.To !== 'USD') {
        let a = ((ANS_to.amount / Pkr_Rate) * Other_Rate).toFixed(4);
        console.log(chalk.cyan.bold(`You Will get ${a} ${ANS_to.To}`));
    }
    console.log(rate);
}
